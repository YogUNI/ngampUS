      ...
      <div>
        <div className="mb-4 rounded-2xl border border-[var(--line)] bg-white p-3 sm:p-4">
          ...
          <form className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            ...
            <select name="semester_id" defaultValue={filters.semester_id || ""} className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-sm"><option value="">Semua semester</option>{semesters?.map((semester) => <option key={semester.id} value={semester.id}>{semester.nama_semester}</option>)}</select>
            <select name="organization_id" defaultValue={filters.organization_id || ""} className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-sm"><option value="">Semua organisasi</option>{organizations?.map((organization) => <option key={organization.id} value={organization.id}>{organization.nama_organisasi}</option>)}</select>
            <select name="kategori" defaultValue={filters.kategori || ""} className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-sm"><option value="">Semua kategori</option><option value="kuliah">Kuliah</option><option value="organisasi">Organisasi</option><option value="lomba">Lomba</option><option value="event">Event</option><option value="lainnya">Lainnya</option></select>
            <select name="prioritas" defaultValue={filters.prioritas || ""} className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-sm"><option value="">Semua prioritas</option><option value="tinggi">Tinggi</option><option value="sedang">Sedang</option><option value="rendah">Rendah</option></select>
        ...
        {calendar ? <CalendarView activities={activities ?? []}/> : <div className="rounded-2xl border border-[var(--line)] bg-white p-5 sm:p-6">
          ...
          <div className="mt-5 space-y-2">{activities?.length ? activities.map((activity) => <article key={activity.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-[var(--line)] p-3 transition hover:border-[#b9ddc6] sm:flex-nowrap">
            ...
            <span className={`order-3 rounded-lg px-2 py-1 text-xs font-bold sm:order-none ${activity.prioritas === "tinggi" ? "bg-[#fff0ec] text-[#b93c21]" : activity.prioritas === "rendah" ? "bg-[#eaf6ee] text-[#17613e]" : "bg-[#f7f8f5] text-[var(--muted)]"}`}>{activity.prioritas}</span>
            <form action={updateActivityStatus} className="order-4 flex items-center gap-1 sm:order-none"><input name="id" type="hidden" value={activity.id}/><select aria-label={`Status ${activity.judul}`} name="status" defaultValue={activity.status} className="rounded-lg border border-[var(--line)] bg-white px-2 py-1.5 text-xs font-semibold"><option value="belum_mulai">Belum mulai</option><option value="on_progress">Berjalan</option><option value="selesai">Selesai</option></select><button className="rounded-lg px-2 py-1.5 text-xs font-bold text-[var(--brand)] hover:bg-[#dcefe4]">Simpan</button></form>
