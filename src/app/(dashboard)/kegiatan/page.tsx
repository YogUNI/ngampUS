...
export default async function ActivitiesPage({ searchParams }: { searchParams: Promise<ActivityFilters> }) {
  ...
  return <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:px-10">
    <header className="flex flex-wrap items-end justify-between gap-4">
      ...
      <ActivityForm semesters={(semesters ?? []).map((semester) => ({ id: semester.id, name: semester.nama_semester, active: semester.is_active }))} organizations={(organizations ?? []).map((organization) => ({ id: organization.id, name: organization.nama_organisasi }))} programs={mappedPrograms}/>
    </header>
      ...
      <div>
        <div className="mb-4 rounded-2xl border border-[var(--line)] bg-white p-3 sm:p-4">
          ...
          <form className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            ...
            <input type="text" name="q" defaultValue={filters.q || ""} placeholder="Cari judul..." className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-sm outline-none focus:border-[var(--brand)]"/>
