...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <main className="min-h-screen bg-[#f7f8f5]"><MobileTopbar/><div className="mx-auto flex min-h-screen max-w-[1600px]"><div className="hidden md:block"><Sidebar name={profile?.full_name || user.email?.split("@")[0] || "Mahasiswa"} activeSemester={activeSemester?.nama_semester}/></div><section className="min-w-0 flex-1">{children}</section></div></main>;
}
