...
export function AuthAside() {
  return <aside className="relative hidden overflow-x-hidden overflow-y-auto bg-[#173f2c] p-10 text-white lg:flex lg:flex-col"><div className="absolute -right-24 top-20 h-72 w-72 rounded-full bg-[#c8ef70]/10 blur-3xl"/><Link className="relative font-display text-2xl font-extrabold" href="/">nGamp<span className="text-[#a6dabb]">US</span></Link><div className="relative my-12"><div className="mb-7 flex items-center gap-2 text-sm font-bold uppercase tracking-[.14em] text-[#b4d8c1]"><Sparkles size={16} className="text-[#c8ef70]"/> Campus identity, reimagined</div><div className="relative mx-auto mb-10 w-[280px] rotate-[-4deg] rounded-2xl border border-[#b4d8c1]/35 bg-[#26563f] p-4 shadow-2xl shadow-black/20"><div className="flex items-start gap-3"><div className="grid h-12 w-12 place-items-center rounded-xl bg-[#dcefe4]/20 text-[#c8ef70]"><ScanFace size={24}/></div><div className="space-y-1.5 text-[10px] font-bold tracking-[.14em] text-[#dcefe4]"><p>NAMA MAHASISWA</p><p>PROGRAM STUDI</p><p className="text-[#c8ef70]">STATUS: AKTIF</p></div></div><div className="mt-7 flex h-7 items-end gap-1 opacity-70">{Array.from({ length: 28 }, (_, index) => <span key={index} className="w-1 rounded-t bg-[#dcefe4]" style={{ height: `${8 + (index * 17) % 18}px` }}/>)}</div></div><h2 className="font-display max-w-md text-5xl font-extrabold leading-[1.03] tracking-[-.05em]">Lebih hadir untuk hal yang benar-benar penting.</h2><p className="mt-6 max-w-sm leading-7 text-[#c7dbce]">Biarkan nGampUS menjaga struktur, supaya energimu tetap untuk berproses.</p></div><p className="relative pb-2 text-sm text-[#9eb9a7]">Personal workspace for campus life.</p></aside>;
}
